...
public:
    ...
    static void next (state &state_, re_token *token_)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                case '/':
                    throw runtime_error ("Lookahead ('/') is "
                        "not supported yet.");
                    break;
                default:
                    token_->_type = CHARSET;
                    ...
                    {
                        char_type upper_ = std::toupper (ch_, state_._locale);
                        char_type lower_ = std::tolower (ch_, state_._locale);
...
private:
    ...
    static void escape (state &state_, string_token &token_)
    {
        ...
        if (str_)
        {
            char_state state2_ (str_ + 1, str_ + str_len_, state_._flags,
                state_._locale, false);
    ...
    static void open_curly (state &state_, re_token *token_)
    {
        if (state_.eos ())
        {
            throw runtime_error ("Unexpected end of regex "
